...
const apiBuilder: SmartlingApiClientBuilder = new SmartlingApiClientBuilder()
    .setLogger(console as unknown as Logger)
...
(async () => {
    try {
        ...
        const job: JobDto = await jobsApi.createJob(projectId, createJobParams);
        ...
        const filesInfo: Array<{fileName: string, fileUri: string}> = filesList
            .map(fileName => ({
                fileName,
        ...
        const batch: BatchDto = await batchesApi.createBatch(projectId, createBatchParams);
        ...
        do {
            ...
            const jobProgress: JobProgressDto = await jobsApi.getJobProgress(projectId, job.translationJobUid, jobProgressParams);
